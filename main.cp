...
void parse_parm(char *input_str, char *output_str, int index){  //given void function
    ...
    int i = 0;     //counter for when while loops nth times (index) then wont meet parameterts 
    ...
    while (output_str != NULL && i != index) { 
        char a = *output_str;               //stores where the currect *output_str points to char a
...
int main ()
{
    char input_str[] ="1,2,3,c,+,+";        //declared string seperated by commas
    char *output_str;
